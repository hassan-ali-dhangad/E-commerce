...
const CreateProduct = () => {
  ...
  // Form state mapped directly to Mongoose Schema
  ...
  // Fetch Categories for backend ID mapping
  useEffect(() => {
  ...
  // Revoke Blob URLs on unmount to prevent browser memory leaks
  useEffect(() => {
    return () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Prepare FormData payload for Multer/Backend upload handling
    ...
    // Transform string inputs to arrays for schema array fields
    const parseList = (str) =>
      str
        .split(/[\n,]/)
        .map((item) => item.trim())
    ...
    parseList(formData.features).forEach((item) => data.append("features", item));
    ...
    // Append primary image single key and gallery images array key
